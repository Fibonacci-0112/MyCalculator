...
- **State Income Tax** — Covers all 50 states and DC with state-specific calculators:
  ...
  - **Custom formula states** — Alabama (graduated + dependents + federal deduction), Arkansas (DFA formula method), California (Method B, EDD DE 44 withholding tables + SDI), Colorado (flat 4.4% + DR 0004 Table 1 allowance + FMLI), Connecticut (TPG-211 table-driven withholding + PFMLI), Illinois (flat 4.95% + IL-W-4 allowances), Oklahoma (OW-2)
  - **Percentage method states** — 34 states using an annualized graduated bracket approach with state-specific standard deductions, allowances, and tax brackets
...
│   ├── Views/                 # XAML pages (Inputs, Results, Compare, Saved Paychecks)
│   ├── ViewModels/            # MVVM view models (Calculator, SavedPaychecks, StateField, DeductionItem)
...
│   ├── Models/                # PaycheckInput, PaycheckResult, Enums, Deduction, AnnualProjection, CalculationScenario, SavedPaycheck
...
│   ├── Data/                  # JSON tax bracket tables (IRS 15-T, OK OW-2, CA Method B, AR, CO DR 0004, CT TPG-211)
│   └── Tax/
...
| **Flat Rate** | PA (3.07%) |
| **Custom Formula** | AL (graduated + dependents + federal deduction), AR (DFA formula method), CA (Method B, EDD DE 44 + SDI), CO (flat 4.4% + DR 0004 Table 1 allowance + FMLI), CT (TPG-211 table-driven + PFMLI), IL (flat 4.95% + IL-W-4 allowances), OK (OW-2 percentage tables) |
| **Annualized Percentage Method** | AZ, DC, DE, GA, HI, IA, ID, IN, KS, KY, LA, MA, MD, ME, MI, MN, MO, MS, MT, NC, ND, NE, NJ, NM, NY, OH, OR, RI, SC, UT, VA, VT, WI, WV |

## UI Overview

The app uses flyout navigation with four main sections:
